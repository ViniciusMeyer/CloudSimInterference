...
public class IntContainerDataCenter extends SimEntity {
     ...
     * @throws Exception
     */
    public IntContainerDataCenter(
            String name,
            IntContainerDataCenterCharacteristics characteristics,
            IntContainerVmAllocationPolicy vmAllocationPolicy,
            IntContainerAllocationPolicy containerAllocationPolicy,
...
//                    Log.printConcatLine("Assigning the container#" + container.getUid() + "to VM #" + containerVm.getUid());
    ...
    protected void processCloudletMove(int[] receivedData, int type) {
        ...
        if (cl == null) {// cloudlet doesn't exist
            ...
            if (destId == getId()) {
                IntContainerVm containerVm = getVmAllocationPolicy().getHost(vmDestId, userId).getContainerVm(vmDestId, userId);
                if (containerVm == null) {
                    failed = true;
                } else {
