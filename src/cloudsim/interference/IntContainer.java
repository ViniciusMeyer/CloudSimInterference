...
import cloudsim.interference.cloudlet.IntContainerCloudletScheduler;
...
public class IntContainer {
	...
	//private IntContainerCloudlet cloudlet; // adapt
...
//    added from the power Vm
	...
	public IntContainer(int id, int userId, double mips, int numberOfPes, String containerManager,
	 ...
	 */
	/*
	 * public double updateContainerProcessing(final double currentTime, final
	 * List<Double> mipsShare) { double time =
	 * updateContainerProcessing1(currentTime, mipsShare); if (currentTime >
	 * getPreviousTime() && (currentTime - 0.2) % getSchedulingInterval() == 0) {
	 * double utilization =
	 * getTotalUtilizationOfCpu(getContainerCloudletScheduler().getPreviousTime());
	 * if (CloudSim.clock() != 0 || utilization != 0) {
	 * addUtilizationHistoryValue(utilization); } setPreviousTime(currentTime); }
	 * return time; }
	 */
	...
	public double updateContainerProcessing(double currentTime, List<Double> mipsShare) {
		...
		if (mipsShare != null) {
			...
			// Log.printLine(" eeeeeeeeeeee CONTAINERupdateContProcess " + x + ", currentime
			// "+ currentTime + ", MipsShare "+mipsShare);
			return x;
	...
	//public IntContainerCloudlet getCloudlet() {  // adapt
	//	return cloudlet;
	//}

	//public void setCloudlet(IntContainerCloudlet cloudlet) {  // adapt
	...
	public double getTotalUtilizationOfCpu(double time) {
		// Log.printLine("Container: get Current getTotalUtilizationOfCpu"+
		// getContainerCloudletScheduler().getTotalUtilizationOfCpu(time));
	...
	public double getTotalUtilizationOfCpuMips(double time) {
		// Log.printLine("Container: get Current
		// getTotalUtilizationOfCpuMips"+getTotalUtilizationOfCpu(time) * getMips());
	 ...
	 * @post $none
	 */
	public void setVm(IntContainerVm vm) {
		this.vm = vm;
	}
